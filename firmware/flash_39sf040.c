...
static uint16_t readSiliconID(void)
{
	...
	// Step 1: Write AAh to address 5555h
	cartWrite(0x05555, 0xAA);
	// Step 2: Write 55h to address 2AAAh
	cartWrite(0x02AAA, 0x55);
	// Step 3: Write 90h to address 5555h
	cartWrite(0x05555, 0x90);
...
static void chipErase(void)
{
	...
	// Step 2: Write 55h to address 2AAAh
	cartWrite(0x02AAA, 0x55);
	// Step 3: Write 80h to address 5555h
	cartWrite(0x05555, 0x80);
	// Step 4: Write AAh to address 5555h
	cartWrite(0x05555, 0xAA);
	// Step 5: Write 55h to address 2AAAh
	cartWrite(0x02AAA, 0x55);
	// Step 6: Write 10h to address 5555h
	cartWrite(0x05555, 0x10);
...
static void programBytes(uint16_t cartAddr, uint8_t *data, int len)
{
	while (len--) {
		...
		// Step 2: Write 55h to address 2AAAh
		cartWrite(0x02AAA, 0x55);
		// Step 3: Write A0h to address 5555h
		cartWrite(0x05555, 0xA0);
...
struct flashops flash_39sf040_ops = {
